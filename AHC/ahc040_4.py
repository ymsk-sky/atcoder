...
def solve() -> None:
    ...
    for _ in range(T - 1):
        ...
        for p, (w, h) in enumerate(whl):
            ...
            """回転方向を決定"""
            # 通常と回転を試して良いほうを採用
            new_rect_r0_src = Rect(w, h)
            new_rect_r0 = Rect(int(w * 1.1) + int(abs(random.gauss(0, sigma))), h)  # 横幅に余裕を持たせて探索
            new_point_r0 = find_BL_point(placements, new_rect_r0, out_box)
            tmp_r0 = new_point_r0.y + new_rect_r0.height

            new_rect_r1_src = Rect(h, w)
            new_rect_r1 = Rect(int(h * 1.1) + int(abs(random.gauss(0, sigma))), w)  # 横幅に余裕を持たせて探索
            new_point_r1 = find_BL_point(placements, new_rect_r1, out_box)
            tmp_r1 = new_point_r1.y + new_rect_r1.height

            m_r0 = max(height_max, tmp_r0)
            m_r1 = max(height_max, tmp_r1)
            if m_r0 < m_r1:
                r = 0
                new_rect = new_rect_r0_src
                new_point = new_point_r0
            elif m_r0 > m_r1:
                r = 1
                new_rect = new_rect_r1_src
                new_point = new_point_r1
            else:
                if tmp_r0 <= tmp_r1:
                    r = 0
                    new_rect = new_rect_r0_src
            ...
            height_max = max(height_max, new_point.y + new_rect.height)
            res.append((p, r, "U", b))
        fin_time = time()
        W, H = query(res=res)
